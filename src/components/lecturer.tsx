...
export default function LecturerComponent() {
  ...
  const handleSelectToggle = (applicant: Applicant) => {
    setSelectedApplicants((prev) =>
      prev.some((a) => a.id === applicant.id && a.role === applicant.role)
        ? prev.filter((a) => !(a.id === applicant.id && a.role === applicant.role))
        : [...prev, applicant]
    );
  };
  ...
  return (
    <div className="flex flex-col gap-8 p-4">
      ...
      {/* link to stats */}
      <Link href="/lecturer/graph" className="text-blue-500 underline text-sm mt-2 self-start">
      ...
      {uniqueSelectedApplicants.length > 0 && selectedCourse && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <RankingEditor
            ...
            role="tutor"
            selectedApplicants={uniqueSelectedApplicants.filter((a) => a.role.toLowerCase() === "tutor")}
            applicants={uniqueSelectedApplicants.map((a) => a.id)}
          />
          <RankingEditor
            course_code={selectedCourse}
            role="lab assistant"
            selectedApplicants={uniqueSelectedApplicants.filter((a) => a.role.toLowerCase() === "lab assistant")}
            applicants={uniqueSelectedApplicants.map((a) => a.id)}
          />
        </div>
      )}
